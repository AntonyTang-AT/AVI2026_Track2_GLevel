...
def mixup_temporal_chunk(chunk_flat, labels, B, T, alpha=0.2):
    """特征为 [B*T, D] 时在样本维做 MixUp（同一 λ 作用于该样本所有时间步）。"""
    device = chunk_flat.device
    D = chunk_flat.size(-1)
    ...
    return mixed_feats.view(B * T, D), mixed_labels
...
def train_model(model, train_loader, criterion, optimizer, device, mixup_prob=0.5, mixup_alpha=0.2):
    model.train()
    total_loss = 0
    train_bar = tqdm(train_loader, desc="Training", leave=False)
    for features, mask, labels, _sample_ids in train_bar:
        features = {k: v.to(device) for k, v in features.items()}
        audio_feat = features['audio']
        video_feat = features['video']
        text_feat = features['text']
        hand_feat = features['hand']
        ...
        if use_mixup:
            chunk_flat, target = mixup_temporal_chunk(chunk_flat, labels, B, T, alpha=mixup_alpha)
        else:
            target = labels
        outputs = model.forward_heads(chunk_flat, B, T)
...
def test_model(model, test_loader, device, columns, output_csv_path, test_csv_path):
    ...
    with torch.no_grad():
        for features, mask, ids in test_bar:
            ...
            text_feat = features['text']
            hand_feat = features['hand']
            outputs = model(audio_feat, video_feat, text_feat, hand_feat)
            outputs = outputs * 4 + 1
...
def evaluate_model(model, loader, criterion, device, columns, is_Test=False):
    ...
    with torch.no_grad():
        for features, mask, labels, _sample_ids in val_bar:
            features = {k: v.to(device) for k, v in features.items()}
            audio_feat = features['audio']
            video_feat = features['video']
            text_feat = features['text']
...
def save_args(args, save_dir="./args_log"):
    ...
    args_file1 = os.path.join(log_dir, f"args_{timestamp}.json")
...
def main():
    ...
    parser.add_argument('--transcript_dir', type=str, default='', help='每题转写 .txt，与特征同名：{id}_{q}.txt')
    parser.add_argument('--wav_dir', type=str, default='', help='原始 wav，与音频特征同名：{id}_{q}.wav')
    # 默认对齐 AVI2026 /data/AVI2026/train_feature 常见 .npy（可用 tools/check_feature_shapes.py 核对）
    ...
    if args.only_test:
        ...
        model.eval()
        test_model(model, test_loader, device, args.label_col, output_csv_path=args.test_output_csv, test_csv_path=args.test_csv)
    ...
    for epoch in trange(args.num_epochs, desc="Epochs", ncols=100):
        ...
        if early_stop_counter >= args.early_stop_patience:
            tqdm.write(f"Early stopping at epoch {epoch+1} (no val improvement for {args.early_stop_patience} epochs).")
            break
