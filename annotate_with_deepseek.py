...
若不存在则回退到 ${SUPERLU_DATASET:-/data/Super-Lu/dataset}/…（与本仓库 vote_train 一致）。

示例：
  python annotate_with_deepseek.py --dry-run
  python annotate_with_deepseek.py --warmup-only --context-cache ./deepseek_context_cache.json
  python annotate_with_deepseek.py --resume --output ./deepseek_annotations_v2.json

关闭「先理解标签」、退回纯规则：
  python annotate_with_deepseek.py --no-labeled-context
...
DEFAULT_OUT = "deepseek_annotations.json"
DEFAULT_CONTEXT_CACHE = "deepseek_context_cache.json"
